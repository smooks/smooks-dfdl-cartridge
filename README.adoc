...
`+dfdl:parser+` is a reader. Its `+schemaURI+` attribute references the DFDL schema driving the parsing behaviour. From the above _input.csv_, `+dfdl:parser+` generates the event stream:
