...
`+dfdl:parser+` is a reader where its `+schemaURI+` attribute references the DFDL schema driving the parsing behaviour. From _input.csv_, `+dfdl:parser+` generates the event stream:
