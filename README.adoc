...
Many DFDL schemas are freely available from the https://github.com/dfdlschemas[DFDL Schemas for Commercial and Scientific Data Formats GitHub repository]. However, should you decide to author your own schemas, we strongly urge you to gain a good understanding of DFDL first. Resources to get started with DFDL include:

* https://ogf.org/ogf/doku.php/standards/dfdl/dfdl[Open Grid Forum DFDL page]
* https://lists.apache.org/list.html?users@daffodil.apache.org[Apache Daffodil user mailing list]
...
DFDL's transformation features should not be conflated with mapping. As highlighted in the validation guidance, a DFDL schema focuses on the physical format of the data. While it is certainly possible to map data in DFDL using https://daffodil.apache.org/docs/dfdl/#_Toc62570192[hidden group] elements and so on, if the data needs to be viewed in a very distinct way, then we recommend that the mapping happens after parsing. One should consider (1) parsing the source first with DFDL, (2) mapping the streaming infoset (e.g., with XSLT, JavaBean cartridge, FreeMarker, etc...), and then (3) feeding the mapped result to the target consumer. A possible solution is to leverage the https://www.smooks.org/documentation/#rewrite[core:rewrite] reader in combination with a https://www.smooks.org/documentation/#templating[FreeMarker visitor], within a pipeline, to map the result as demonstrated in the https://github.com/smooks/smooks-examples/tree/v5/pipelines[pipelines example].
